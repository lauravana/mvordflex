mvordflex.fit <- function(rho){
  ...
  #   }
    ...
    # })
  if(is.null(rho$start.values$theta)){
    rho$start.values$theta <- get_start_values(rho)[seq_len(sum(rho$npar.theta.opt))]
  }
  if(is.null(rho$start.values$beta)){
    rho$start.values$beta <- get_start_values(rho)[sum(rho$npar.theta.opt) + seq_len(rho$npar.betas)]
  }

    if(is.null(rho$start.values$error.structure)){
      rho$start.values$error.structure <- start_values(rho$error.structure, rho$y)
    }
    ...
    #                unlist(rho$start.values$error.structure))
    rho$start <- c(unlist(rho$start.values$theta), #gammas
                   unlist(rho$start.values$beta),#betas
                   unlist(rho$start.values$error.structure))
  }
  ...
  if (rho$evalOnly) {
    ...
    #######
    # names_rho <- names(rho)
    # names_rho_opt <- c("npar.thetas", "npar.betas", "error.structure", "build_error_struct", "constraints_mat", "transf_thresholds",
    #                    "ndim", "threshold.values", "ntheta", "ind.thresholds", "thold_correction", "mat_center_scale", "contr_theta",
    #                    "inds.cat", "inf.value", "y", "offset", "n", "ind_univ", "ind_kl", "weights", "combis_fast", #"combis",
    #                    "indjbeta", "XcatU", "XcatL",
    #                    "dummy_pl_lag", "solver", "control", "start", "link")
    # rho_non_opt <- rho[names_rho[!(names_rho %in% names_rho_opt)]]
    # #rho <- rho[names_rho_opt]


    # rho$link$F_uni <-rho_non_opt$link$F_uni
