...
###############################
start_values.cov_general <- function(eobj, y=NULL) {
...
#################################
start_values.cor_general <- function(eobj, y = NULL) {
  ## builds starting values for the correlation structure
  tmp <- double(attr(eobj, "npar"))
  ## TODO for the given values

  ndim_j <- attr(eobj, "ndim")
  covar <- attr(eobj, "covariate")
  nlev <- NCOL(covar)
...
#### Methods for cor_ar1 ####
#############################
start_values.cor_ar1 <- function(eobj, y=NULL) {
  ## builds starting values for the correlation structure
  if (eobj$fixed) {
    tmp <- double(0)
  } else {
...
##############################
start_values.cor_equi <- function(eobj, y=NULL) {
  ## builds starting values for the correlation structure
  if (eobj$fixed) {
    tmp <- double(0)
  } else {
