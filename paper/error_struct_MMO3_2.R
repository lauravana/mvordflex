...
make_Q_cor_MMO3 <- function(tpar, eobj){
  ...
  # PSI matrix and unconditional variance of errors
  if (npar_psi == 0) {
  ...
  if(eobj$Psi.diag & npar_psi != 0) {
    ...
    sigma0 <- sigma
    #sigma0 <-  matrix(1/(1 - c(tcrossprod(xi))) * c(sigma),
    #                  ncol = ndim_j)

  }
  if(!eobj$Psi.diag & npar_psi != 0) {
    psi <-  make_stationary_psi(tpar_psi)
    sigma0 <- matrix(solve(diag(ndim_j^2) - kronecker(psi, psi), c(sigma)),
                     ncol = ndim_j)
  }

  # ar_blocks <- vector("list", ndim_t)
  # for (tt in 0:(ndim_t - 1)) {
  #   ar_blocks[[tt + 1]] <- sigma0 %*% (psi ^ tt) ##only for diagonal psi
  # }
  ar_blocks <- lapply(0:(ndim_t - 1), function(tt) {
    s_ar <- psi ^ tt
    crossprod(s_ar, sigma)
  })
 # ar_blocks <- lapply(0:(ndim_t - 1), function(tt) sigma %*% (psi ^ tt))
  print(psi)
  print(sigma)
...
build_error_struct.cor_MMO3 <- build_error_struct.cor_MMO3_cross <-
  ...
  function(eobj, tpar)
  {
    ...
    corr_pars <- Q[lower.tri(Q)]
    sdQ <-  rep(1, ndim) #sqrt(diag(Q)) # rep(1, ndim)
...
finalize_fun.cor_MMO3 <- finalize_fun.cor_MMO3_cross <- finalize_fun.cor_MMO3_ar1 <- function(eobj, tpar) {
  if (eobj$fixed){
    ...
    if (eobj$formula == ~1) {
      ...
      names.psi <- if (eobj$Psi.diag) paste0("psi ", 1:ndim_j) else
        c(outer(1:ndim_j, 1:ndim_j, function(x,y) paste("psi", x, y)))
